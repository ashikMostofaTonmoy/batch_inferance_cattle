...
def predict(side_fname, rear_fname):
    try:
        ...
        # det_checkpoint = 'models/v1/det/yolox_x_8x8_300e_coco_20211126_140254-1ef88d67.pth'
        ...
        model = init_segmentor(
            seg_config_file, seg_checkpoint_file, device='cpu')

        rear_pose_model = init_pose_model(
            rear_pose_config, rear_pose_checkpoint)
        side_pose_model = init_pose_model(
            side_pose_config, side_pose_checkpoint)
        ...
        rear_person_results = process_mmdet_results(
            rear_mmdet_results, cat_id=20)
        side_person_results = process_mmdet_results(
            side_mmdet_results, cat_id=20)
        ...
        show_result_pyplot(model, side_img, side_seg_result,
                           get_palette('voc'))
        sticker = cattle = bg = 0
        ...
        if sticker < 100:
            ...
            res = {"weight": predicted_cattle_weight,
                   "ratio": cattle/sticker, "remarks": status}
            # os.remove(side_img)
            # os.remove(rear_img)
            return res
        # inference pose
    ...
    # KPT rear and side
        ...
        if (int(sidex1) > (side_im_width/2)):
            ...
            seg_crop = segImg.crop(
                (int(sidex8), 0, side_im_width, side_im_height))
            side_p1, side_p2 = adjust(seg_crop)
            side_height = y_distancae(side_p1, side_p2)
        ...
        side_Length_shoulderbone = round(
            ((sidey2-sidey1)**2+(sidex2-sidex1)**2)**0.5)
        ...
        if(cattle/sticker < 50):
            predicted_cattle_weight = predicted_cattle_weight - \
                (cattle/sticker)*0.68
            res = {"weight": predicted_cattle_weight,
                   "ratio": cattle/sticker, "remarks": status}
            # os.remove(side_img)
        ...
        try:
            ...
            status = "ok"
            predicted_cattle_weight = ((cattle+sticker+rear_px)/sticker)
            res = {"weight": predicted_cattle_weight,
                   "ratio": cattle/sticker, "remarks": status}
            return res
        except:
